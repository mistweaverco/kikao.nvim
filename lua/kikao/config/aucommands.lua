...
local vim_enter_cb = function(config, data, session_file_path)
  ...
  if vim.fn.filereadable(session_file) == 1 then
    ...
    if session_file_size > 0 then
      ...
      local ok, err = pcall(function() vim.cmd("silent! source " .. session_file) end)
      if not ok then
      ...
      if data.file then vim.cmd("e " .. data.file) end
    end
  end
end
...
M.setup = function(config)
  ...
  if config.session_file_path == nil then
    ...
    if session_file_path_info == nil then return end
  ...
  vim.api.nvim_create_autocmd("VimEnter", {
    callback = function(data)
      ...
      if Utils.is_diffconflicts_buffer() then return end
      -- Call the session restoration function
      pcall(function() vim_enter_cb(config, data or {}, session_file_path) end)
    end,
    group = augroup,
    nested = true,
  })
  ...
  vim.api.nvim_create_autocmd("VimLeave", {
    callback = function()
      ...
      if has_diffconflicts then return end
